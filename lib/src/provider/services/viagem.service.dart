...
class ViagemService {
  ...
  Future<Emergencias> save(Emergencias entity) async {
  ...
  Future<void> cancelAllViagensPassageiroAberta(String idPassageiro) async {
    ...
    if (!query.documents.isEmpty) {
      List<Emergencias> listaViagensAberta = query.documents
          .map((result) => Emergencias.fromSnapshotJson(result))
          .toList();
  ...
  Future<Stream<QuerySnapshot>> startViagem(Emergencias entity) async {
    await save(entity);
  ...
  Future<Emergencias> getStreamViagemById(String id) async {
    final CollectionReference _dbs = Firestore.instance.collection(_baseUrl);
    ...
    return query.documents.isEmpty
        ? null
        : Emergencias.fromSnapshotJson(query.documents[0]);
  }
  ...
  Future<Emergencias> getViagemAbertaByUsuario(String idPassageiro,
  ...
  Future<List<Emergencias>> getViagensByPassageiro(String idPassageiro) async {
    final CollectionReference _dbs = Firestore.instance.collection(_baseUrl);
  ...
  Future<List<Emergencias>> getViagensByMotoristaConcluida(
  ...
  Future<List<Emergencias>> getViagensByMotoristaConcluidaWithDataInicialFinal(
  ...
  Future<List<Emergencias>> getViagensByMotorista(String idMotorista) async {
    final CollectionReference _dbs = Firestore.instance.collection(_baseUrl);
