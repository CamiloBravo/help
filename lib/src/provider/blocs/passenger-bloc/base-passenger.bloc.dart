...
class BasePassageiroBloc extends BlocBase {
  ...
  final BehaviorSubject<Emergencias> _viagemController =
      new BehaviorSubject<Emergencias>();

  Stream<Emergencias> get viagemFlux => _viagemController.stream;

  Sink<Emergencias> get viagemEvent => _viagemController.sink;
...
/*fim provider mapa*/
  ...
  Future<void> orchestration() async {
    HomePassageiroBloc _homePassageiroBloc = BlocProvider.getBloc<HomePassageiroBloc>();
    StepPassengerHome _stepHome = await _homePassageiroBloc.stepProcessoFlux.first;
    ...
    switch (_stepHome) {
      case StepPassengerHome.Start:
        ...
        break;
      case StepPassengerHome.ConfirmPrice: //aqui hice un cambio que es que no aparezca el mapa de la ruta
  ...
  Future cancelarCorrida() async {
    ...
    Emergencias viagem = await viagemFlux.first;
  ...
  // Adiciona un punto en el provider del mapa
