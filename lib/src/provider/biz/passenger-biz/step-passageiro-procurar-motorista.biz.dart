...
class StepPassageiroProcurarMotorista {
  ...
  Future start() async {
    ...
    Emergencias viagem = await _basePassageiroBloc.viagemFlux.first;

    viagem.PassageiroEntity = passageiro;

    Emergencias viagemResult = await _viagemService.getViagemAbertaByUsuario(
    ...
    _streamAllAbertaViagem = stream.listen((data) {
      data.documentChanges.forEach((change) async {
        ...
        if (!statusSemaforo) {
          ...
          var resultViagem = Emergencias.fromSnapshotJson(change.document);
          ...
          if (resultViagem.Status == TravelStatus.DriverPath) {
            ...
            _basePassageiroBloc.viagemEvent.add(Emergencias());
  ...
  Future rotaCorridaIniciada(Emergencias viagem) async {
    ...
    LatLng origen = LatLng(6.196875, -75.600107);
    LatLng destino = LatLng(6.191585, -75.592635);
  ...
  Future rotaMotoristaAteUsuario(Emergencias viagem) async {
    ProviderMapa providerMapa = ProviderMapa(
