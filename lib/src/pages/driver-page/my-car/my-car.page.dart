...
class _MyCarPageState extends State<MyCarPage> {
  ...
  Future<void> _salvarInfo() async {
    ...
    if (tipoSelecionado == null || tipoSelecionado == '') {
      ShowSnackBar.build(_scaffoldKey, 'Need to select car type.', context);
      return;
    }

    if (marcaSelecionada == null || marcaSelecionada == '') {
      ShowSnackBar.build(_scaffoldKey, 'Need to select a car brand.', context);
      return;
    }

    if (modeloSelecionada == null || modeloSelecionada == '') {
      ShowSnackBar.build(_scaffoldKey, 'Need to select a car model.', context);
      return;
    }

    if (anoSelecionada == null || anoSelecionada == '') {
      ShowSnackBar.build(_scaffoldKey, 'Must select car year.', context);
      return;
    }

    if (corSelecionada == null || corSelecionada == '') {
      ShowSnackBar.build(_scaffoldKey, 'Need to select car color.', context);
      return;
    }
    ...
    if (_textPlacaName == null ||
        ...
        (_textPlacaName.value.text.replaceAll('-', '').length != 7) ||
    ...
    ShowSnackBar.build(_scaffoldKey, 'Successfully saved car data.', context);
  }
