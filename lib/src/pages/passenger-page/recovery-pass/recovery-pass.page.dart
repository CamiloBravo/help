  ...
  Future<void> _functionRecoveryEmail(String email) async {
    if (_formKey.currentState.validate()) {
      ...
      _auth.recoveryPassword(email: email).then((r) {
        ...
        ShowSnackBar.build(
            _scaffoldLoginKey, 'El correo ya fue enviado', context);
      }, onError: (ex) {
        _baseBloc.loadingStatusEvent.add(false);
        switch (ex.code) {
          case 'ERROR_USER_NOT_FOUND':
            ShowSnackBar.build(
                _scaffoldLoginKey, 'Lo sentimos, no se encontró el correo', context);
            break;
          default:
            ShowSnackBar.build(
                _scaffoldLoginKey, 'Lo sentimos, un error ha ocurrido', context);
            break;
        }
      });
    }
