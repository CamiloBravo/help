...
class _SelectSourceDestinyWidgetState
  ...
  //Al iniciar esta pantalla, obtiene los valores del destino y la fuente actual de la pantalla y agrega la entrada
  _load() async {
  ...
  //comprueba si el origen y el destino se han agregado correctamente y si ha comenzado el proceso
  _validaProximaEtapa() async {
    ProviderMapa provider = await _baseBloc.providermapFlux.first;

    LatLng latlng2 =
    LatLng(6.211466, -75.577755);
    _baseBloc.refreshProvider(latlng2,
        'El Poblado', LocalLocation.Destiny);
    destinoController.text = 'El Poblado';
    provider.EnderecoDestino = 'El Poblado';
    ...
    if (provider.EnderecoOrigem.isNotEmpty &&
      ...
      print('entra aqui');
      _googleService
          .getdistancia(provider.LatLngOrigemPoint, latlng2)
          .then((result) async {
        Emergencias viagem = Emergencias(
        ...
        print('Destino: ');
        print(viagem.DestinoEndereco);

        _baseBloc.viagemEvent.add(viagem);
        _homeBloc.tipocarroEvent.add(TipoCarro.Pop);
        TipoCarro tipoCarro = TipoCarro.Pop;
        viagem.TipoCorrida = tipoCarro;
        _homeBloc.stepProcessoEvent.add(StepPassengerHome.SearchDriver);
        await _baseBloc.orchestration();
      });
    }
    else print('Error no hay datos');
  }
...
//                        onChanged: (value) { //Esto no autocompleta ese campo de origen
...
//                                color: Colors.black,
//                              ),
//                              onPressed: () { //Esto tampoco me autocompleta el origen
